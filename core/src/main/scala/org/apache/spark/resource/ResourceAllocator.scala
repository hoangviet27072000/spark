...
private[spark] trait ResourceAllocator {
  ...
  def acquire(addrs: Seq[String]): Unit = {
    addrs.foreach { address =>
      if (!addressAvailabilityMap.contains(address)) {
        throw ExecutionErrors.acquireAnAddressNotExistError(resourceName, address)
      }
      val isAvailable = addressAvailabilityMap(address)
      if (isAvailable > 0) {
        addressAvailabilityMap(address) = addressAvailabilityMap(address) - 1
      } else {
        throw ExecutionErrors.acquireAnAddressNotAvailableError(resourceName, address)
      }
    }
  }
  ...
  def release(addrs: Seq[String]): Unit = {
    addrs.foreach { address =>
      if (!addressAvailabilityMap.contains(address)) {
        throw ExecutionErrors.releaseAnAddressNotExistError(resourceName, address)
      }
      val isAvailable = addressAvailabilityMap(address)
      if (isAvailable < slotsPerAddress) {
        addressAvailabilityMap(address) = addressAvailabilityMap(address) + 1
      } else {
        throw ExecutionErrors.releaseAnAddressNotAssignedError(resourceName, address)
      }
    }
  }
}
