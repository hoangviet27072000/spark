  ...
  override def discoverResource(
    ...
    val result = if (script.isPresent) {
      ...
      if (scriptFile.exists()) {
        ...
        throw ExecutionErrors.resourceScriptNotExistError(scriptFile, resourceName)
      }
    } else {
      throw ExecutionErrors.expectUseResourceButNotSpecifyADiscoveryScriptError(resourceName)
    }
    if (!result.name.equals(resourceName)) {
      throw ExecutionErrors.runningOtherResourceError(script, result.name, resourceName)
    }
    Optional.of(result)
  }
}
